...
import { Appartement, FilterOptions, Immeuble, ImmeubleFormData, Proprietaire } from '../types';
...
const convertTimestamps = (data: any): any => {
  ...
  if (converted.proprietaireActuel?.dateDebut?.toDate) {
    converted.proprietaireActuel.dateDebut = converted.proprietaireActuel.dateDebut.toDate();
  }
  if (converted.proprietaireActuel?.dateFin?.toDate) {
    converted.proprietaireActuel.dateFin = converted.proprietaireActuel.dateFin.toDate();
  }
  
  // Convertir l'historique des propriétaires
  if (converted.historiqueProprietaires) {
    converted.historiqueProprietaires = converted.historiqueProprietaires.map((prop: any) => ({
      ...prop,
      dateDebut: prop.dateDebut?.toDate ? prop.dateDebut.toDate() : prop.dateDebut,
  ...
  if (converted.appartements) {
    converted.appartements = converted.appartements.map((apt: any) => ({
      ...
      locataireActuel: apt.locataireActuel ? {
        ...apt.locataireActuel,
        dateEntree: apt.locataireActuel.dateEntree?.toDate ? apt.locataireActuel.dateEntree.toDate() : apt.locataireActuel.dateEntree,
        dateSortie: apt.locataireActuel.dateSortie?.toDate ? apt.locataireActuel.dateSortie.toDate() : apt.locataireActuel.dateSortie,
      } : undefined,
      historiqueLocataires: apt.historiqueLocataires?.map((loc: any) => ({
        ...loc,
        dateEntree: loc.dateEntree?.toDate ? loc.dateEntree.toDate() : loc.dateEntree,
        dateSortie: loc.dateSortie?.toDate ? loc.dateSortie.toDate() : loc.dateSortie,
      })) || []
    }));
  }
...
export const immeublesService = {
  ...
  async creerImmeuble(data: ImmeubleFormData): Promise<{ success: boolean; id?: string; error?: string }> {
    try {
      const now = new Date();
      ...
      const docRef = await addDoc(collection(db, COLLECTION_NAME), immeubleData);
  ...
  async obtenirImmeubles(filters?: FilterOptions): Promise<{ success: boolean; data?: Immeuble[]; error?: string }> {
    try {
  ...
  async obtenirImmeuble(id: string): Promise<{ success: boolean; data?: Immeuble; error?: string }> {
    try {
  ...
  async modifierImmeuble(id: string, data: Partial<ImmeubleFormData>): Promise<{ success: boolean; error?: string }> {
    try {
      const docRef = doc(db, COLLECTION_NAME, id);
  ...
  async supprimerImmeuble(id: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async obtenirVilles(): Promise<{ success: boolean; data?: string[]; error?: string }> {
    try {
  ...
  async obtenirAppartementsDisponibles(): Promise<{ success: boolean; data?: Array<{id: string, nom: string, immeubleNom: string}>; error?: string }> {
    try {
      const querySnapshot = await getDocs(collection(db, COLLECTION_NAME));
      const appartementsDisponibles: Array<{id: string, nom: string, immeubleNom: string}> = [];
